...
/**
 * Pipeline: ingest (connectors) -> resolve -> graph -> score -> reason/decide.
 * Stages are plain functions over the event-logged store, so any stage can be re-run
 * idempotently. AI appears only after reliable data is established (stage 5 of 5).
 */
export async function runPipeline(db: DB, provider: LLMProvider): Promise<{
  ...
  logEvent(db, 'pipeline', null, { resolve, graph, scores, decision: decision.id, reused: decision.reused });
  return { resolve, graph, scores, decision };
}
